...
double safe_asin(double f) {
  ...
  if ( (fabs(f) - 1.00)  < DP_TOL ) return(M_PI_2);
  else { ERROR("ASIN : invalid argument %f", f); return 0; }
  }
...
void * mycalloc(const char * filename, const int linenr,
  ...
  if(ip) return(ip);
  else { ERROR("CALLOC : failed in file %s at line %d", filename, linenr); return NULL; }
  }
...
void * myrealloc(const char * filename, const int linenr,
  ...
  if(ip) return(ip);
  else { ERROR("REALLOC : failed in file %s at line %d", filename, linenr); return NULL; }
  }
